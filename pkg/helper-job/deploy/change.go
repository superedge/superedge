...
func updateKubeAPIPod(kubeClient *kubernetes.Clientset, pod *v1.Pod) error {
	...
	for { //make sure tunnel-coredns success created
		coredns, err := kubeClient.CoreV1().Services("kube-system").Get(context.TODO(), "tunnel-coredns", metav1.GetOptions{})
		if err == nil {
...
func completeLiteApiServer(kubeClient *kubernetes.Clientset) error {
	edgeCertCM, err := kubeClient.CoreV1().ConfigMaps("kube-system").Get(context.TODO(), constant.EDGE_CERT_CM, metav1.GetOptions{})
	if err != nil {
		return err
	}
...
func isRunningLiteAPIServer(kubeClient *kubernetes.Clientset, nodeName string, retry int) (bool, error) {
	...
	liteAPIServerStatueFunc := func(podName string) (bool, error) {
		pod, err := kubeClient.CoreV1().Pods("kube-system").Get(context.TODO(), podName, metav1.GetOptions{})
		if err != nil {
