...
// List lists all NodeUnits in the indexer.
func (s *nodeUnitLister) List(selector labels.Selector) (ret []*v1alpha1.NodeUnit, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.NodeUnit))
	})
	return ret, err
}

// Get retrieves the NodeUnit from the index for a given name.
func (s *nodeUnitLister) Get(name string) (*v1alpha1.NodeUnit, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("nodeunit"), name)
	}
	return obj.(*v1alpha1.NodeUnit), nil
}
